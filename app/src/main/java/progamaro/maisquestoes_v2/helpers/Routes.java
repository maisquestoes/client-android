package progamaro.maisquestoes_v2.helpers;

/**
 * Created by andremiranda on 30/07/15.
 */
public class Routes {
    public static final String SIGNUP = "http://api2.maisquestoes.com.br/auth/signup/";
    public static final String SIGNIN = "http://api2.maisquestoes.com.br/auth/signin/";
    public static final String ALL_SUBJECTS = "http://api2.maisquestoes.com.br/subjects/";
    public static final String QUESTION = "http://api2.maisquestoes.com.br/questions/";
}
